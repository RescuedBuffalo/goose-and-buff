# Tile prototype — Phase 1 notes

Architecture-pivot rebuild onto an isometric TileMap. Same wave-defense
loop, same data, same logic.

## Run it

Open `godot-tile-prototype/project.godot` in Godot 4.6. Click a tile to
walk Buffalo. Drag cards during prep; drop Charge during a wave. Space
readies early, Q casts the signature.
...
- **Sector orientation.** DIAMOND_DOWN runs east–west diagonally, not
  literally horizontal like the top-down arena.

## Merge questions

1. **Replace or coexist?** Pure logic + data are duplicated across the
   two folders. Fine through one playtest; merge the winner within a
   milestone.
2. **Grid size.** 16×12 fits 1920×1080 but isn't load-bearing. Phase 2
   exploration wants more space; revisit before BUF-128.
3. **Formation control.** Click-to-move invites deliberate movement;
   the next UX question is how the player commands units.
