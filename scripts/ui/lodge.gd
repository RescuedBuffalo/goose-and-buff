extends Control
##
## The Lodge — meta-progression staging area shown between runs.
##
## Two stations on one screen:
##   1. Card unlocks: locked cards in this hero's pool, each with a price tag.
##      Click to spend tokens and add the card to the unlocked pool.
##   2. Deck swaps (a.k.a. the minimum-viable deck builder): unlocked cards
##      with a single toggle that swaps them in for the starter card they
##      replace. Toggling again swaps them back out.
##
## Hero tabs at the top scope both stations to one hero at a time. Token
## balance and per-hero unlocked count surface in the header so the player
## doesn't have to leave the Lodge to see their progress.
##
## Visual language follows hero_select.gd — Night-0 curtain, parchment card
## bodies, faction core/floor accents on the active tab. All values pulled
## from DesignTokens; hero ordering from `Heroes.ORDER` so it matches the
## hero-select wireframe's muscle memory.
...
func _ready() -> void:
	...
	MetaProgress.tokens_changed.connect(_on_meta_changed)
	MetaProgress.hero_changed.connect(_on_hero_meta_changed)
...
func _on_meta_changed(_balance: int) -> void:
	queue_redraw()

func _on_hero_meta_changed(hero_id: String) -> void:
...
func _perform_action(kind: String, card_id: String) -> void:
	match kind:
		"unlock":
			if MetaProgress.unlock_card(_selected_hero, card_id):
				_show_toast("Unlocked.")
			elif not MetaProgress.can_afford_unlock():
				_show_toast("Not enough tokens.")
		"toggle_swap":
			MetaProgress.toggle_in_deck(_selected_hero, card_id)
			# Redraw is driven by the hero_changed signal — no toast needed,
...
func _draw_header() -> void:
	...
	var eyebrow := "BETWEEN RUNS"
	var headline := "The lodge."
	...
	var balance := "%d tokens" % MetaProgress.tokens
...
func _draw_body() -> void:
	...
	for cid in pool:
		if MetaProgress.is_unlocked(_selected_hero, cid):
...
func _draw_action_button(rect: Rect2, card_id: String, kind: String, accent: Color) -> void:
	...
	match kind:
		"unlock":
			label = "Unlock · %d" % MetaProgress.UNLOCK_COST
			enabled = MetaProgress.can_afford_unlock()
			if not enabled:
				label_color = DesignTokens.FG_3
		"toggle_swap":
			if MetaProgress.is_in_deck(_selected_hero, card_id):
...
func _unlocked_count(hero_id: String) -> int:
	var arr: Array = MetaProgress.unlocked.get(hero_id, [])
	return arr.size()
